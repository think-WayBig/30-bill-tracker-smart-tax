...
const DeletedStatements: React.FC = () => {
  ...
  useEffect(() => {
    ;(async () => {
      const all = (await window.electronAPI.loadStatements3()) ?? []
  ...
  const restoreSelected = async () => {
    ...
    try {
      for (const id of ids) {
        ...
        if (!row) continue
        await window.electronAPI.updateStatement3({ ...row, deleted: false })
      }
  ...
  return (
    <Layout title="🗂️ Deleted Statements" financialYear color="#d30043ff">
      <SectionHeader
