...
const DeletedStatements: React.FC = () => {
  ...
  useEffect(() => {
    ;(async () => {
      const all = (await window.electronAPI.loadStatements2()) ?? []
  ...
  const restoreSelected = async () => {
    ...
    try {
      for (const id of ids) {
        ...
        if (!row) continue
        await window.electronAPI.updateStatement2({ ...row, deleted: false })
      }
  ...
  return (
    <Layout title="🗂️ Deleted Statements" financialYear color="#e06100ff">
      <SectionHeader
