...
const DATA_DIR = path.join('../../../smart-tax-dont-touch')
...
ipcMain.handle('save-group', async (_event, group) => {
  try {
    const filePath = path.join(DATA_DIR, 'groups.json')
...
const getBillsPath = () => {
  const filePath = path.join(DATA_DIR, 'bills.json')
  return filePath
}
...
const getStatementsPath = () => {
  const filePath = path.join(DATA_DIR, 'statements.json')
  return filePath
}
...
const getStatements2Path = () => {
  const filePath = path.join(DATA_DIR, 'savings_statements.json')
  return filePath
}

// Create / Save Statement
ipcMain.handle('save-statement2', async (_event, statement: Omit<BankStatementRow, 'id'>) => {
  try {
    const filePath = getStatements2Path()
    const existing: BankStatementRow[] = fs.existsSync(filePath)
      ? JSON.parse(fs.readFileSync(filePath, 'utf-8'))
      : []
...
// Read / Load all Statements
ipcMain.handle('load-statements2', async () => {
  try {
    const filePath = getStatements2Path()
    if (!fs.existsSync(filePath)) return []
    const data = fs.readFileSync(filePath, 'utf-8')
...
// Update Statement
ipcMain.handle('update-statement2', async (_event, updated: BankStatementRow) => {
  try {
    const filePath = getStatements2Path()
    if (!fs.existsSync(filePath)) {
      return { success: false, error: 'Statements file not found.' }
...
// Delete Statement
ipcMain.handle('delete-statement2', async (_event, id: string) => {
  try {
    const filePath = getStatements2Path()
    if (!fs.existsSync(filePath)) {
      return { success: false, error: 'Statements file not found.' }
...
const getStatements3Path = () => {
  const filePath = path.join(DATA_DIR, 'personal_statements.json')
  return filePath
}

// Create / Save Statement
ipcMain.handle('save-statement3', async (_event, statement: Omit<BankStatementRow, 'id'>) => {
  try {
    const filePath = getStatements3Path()
    const existing: BankStatementRow[] = fs.existsSync(filePath)
      ? JSON.parse(fs.readFileSync(filePath, 'utf-8'))
      : []
...
// Read / Load all Statements
ipcMain.handle('load-statements3', async () => {
  try {
    const filePath = getStatements3Path()
    if (!fs.existsSync(filePath)) return []
    const data = fs.readFileSync(filePath, 'utf-8')
...
// Update Statement
ipcMain.handle('update-statement3', async (_event, updated: BankStatementRow) => {
  try {
    const filePath = getStatements3Path()
    if (!fs.existsSync(filePath)) {
      return { success: false, error: 'Statements file not found.' }
...
// Delete Statement
ipcMain.handle('delete-statement3', async (_event, id: string) => {
  try {
    const filePath = getStatements3Path()
    if (!fs.existsSync(filePath)) {
      return { success: false, error: 'Statements file not found.' }
...
const getAuditsPath = () => {
  const filePath = path.join(DATA_DIR, 'audits.json')
  return filePath
}
